...
const { contactSync } = require("../../lib/commerce/contact")
...
async function main(params) {
    ...
    try {
        ...
        logger.info('Calling the contact sync action')
        ...
        */
        ...
        const contactsync = contactSync(oauth, logger)
        const processcontrol = processControl(oauth, logger)

        // call AC API to search if there is any process are running.
        const runningProcesses = await processcontrol.findRunningProcessByTask(constants.CONTACT_SYNC_KK_TO_STG_TASK)
        // console.log(runningProcesses)
        if (runningProcesses.total_count > 0) {
            logger.info("Warning: There are " + runningProcesses.total_count + " contact sync from KK staging to AC staging process running. Please check the log.")
            const response = {
                statusCode: 500,
                body: JSON.stringify({
                    "error": "Warning: There are " + runningProcesses.total_count + " contact sync from KK staging to AC staging process running. Please check the log."
                })
            }
            return response;
        }
        ...
        */
        let process = await processcontrol.insertProcess(
            constants.CONTACT_SYNC_KK_TO_STG_TASK,
            "Retrieving contact from Kinetic to AC staging"
        )
        logger.info(process)
        ...
        */
        ...
        let skip = 0
        let top = constants.CONTACT_SYNC_KK_TO_STG_BATCH_COUNT
        let kkStagingContacts = await kkRestClient.getStagingContact(skip, top)
        // console.log(kkStagingProducts)
        const totalCount = kkStagingContacts["@odata.count"];
        let retrievedCount = kkStagingContacts["value"].length;
        logger.info(totalCount)
        if (totalCount > 0) {
            // Insert the retrieved data from Kinetic API to staging table aio_ac_erp_product_master
            let insertedContacts = await contactsync.insertContact(kkStagingContacts["value"])
            while (retrievedCount < totalCount) {
                skip = skip + top
                kkStagingContacts = await kkRestClient.getStagingContact(skip, top)
                retrievedCount = retrievedCount + kkStagingContacts["value"].length
                // Insert the retrieved data from Kinetic API to staging table aio_ac_erp_product_master
                let insertedContacts = await contactsync.insertContact(kkStagingContacts["value"])
            }
        } else {
            logger.info("No contact data from STAGING API")
        }
        ...
        process.sync_notes = `Total ${retrievedCount} contact records are retrieved from KK STAGING to AC STAGING.`
