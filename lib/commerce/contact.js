...
function contactSync(oauth, logger) {
    const instance = {}

    instance.insertContact = async function (rows) {
        let i = 0;
        const valuesLabels = [];
        const data = {}
        try {
            for (const key in rows) {
                if (rows.hasOwnProperty(key)) {
                    let row = rows[key];
                    i++;
                    row = _formattedRowFieldKeys(row, 'Web_Export_Contacts_');
                    ...
                    const valuesLabel = {
                        ...
                        'per_con_id': row['per_con_id'],
                        'rb_sales_rep_code': row['rb_sales_rep_code'],
    ...
    instance.getContacts = async function (pageSize = 20, currentPage = 1, syncStatuses = ['N']) {
        let searchCriteria = {
            "pageSize": pageSize,
    ...
    _handleError = function (error, syncStatus, syncNotes) {
        if (
            ...
            if (errorMessage.message === 'The contact is already attached.') {
                syncStatus = 'O';
            } else {
                syncStatus = 'F';
            }
