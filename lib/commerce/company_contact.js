...
function companyContactSync(oauth, logger) {
    ...
    instance.getCompanies = async function (pageSize = 20, currentPage = 1, filters = []) {
        let searchCriteria = {
            "pageSize": pageSize,
            "currentPage": currentPage,
            "filterGroups": [
    ...
    instance.updateAdminContactToAc = async function (adminContacts,custAdminContactMapping) {
        ...
        try {
            ...
            const promises = contactParams.map(contactParam => _handleAdminContact(contactParam));
    ...
    instance.updateContactToAc = async function (stgContacts, customerCompanyIdMapping) {

        const contactParams = [];
        const valuesLabels = [];
        const data = {}

        for (const stgContact of stgContacts) {
            let formatted_data = await _reformatDataContact(stgContact);
            let formattedData = formatted_data;
            let params = {};
            let missedRequiredFields = [];
            ...
            if (stgContact['ac_customer_id']) {
                params = {
                    "customer": {
                        "id" :stgContact['ac_customer_id'],
                        "email": 'test.' + formattedData['email'],
                        "firstname": formattedData['firstname'],
                        "lastname": formattedData["lastname"],
                        "website_id": 1,
        ...
        try {
            // Handling contacts in parallel to speed up the process.
            const promises = contactParams.map(contactParam => _handleStgContact(contactParam, customerCompanyIdMapping));
            const results = await Promise.all(promises);
            logger.info('777777' + JSON.stringify(results))
            for (let i = 0; i < stgContacts.length; i++) {
                let syncResult = results.find((itmInner) => itmInner.id === stgContacts[i].id);
                valuesLabels.push({
                        ...stgContacts[i],
                        ...syncResult
                    }
            ...
            // logger.info(resourceUrl);
            const contactProcess = await oauth.post(resourceUrl, data)
            logger.info('66666' + JSON.stringify(contactProcess))
        } catch (error) {
            logger.info('###################' + error?.message);
            let syncResult = _handleError(error, syncStatus, syncNotes);
            valuesLabels.map(item => {
    ...
    _handleAdminContact = async function (contactParam) {
            ...
            try {
             ...
             //  logger.info( '2222233333' +  JSON.stringify(contactInfo))
              ...
              //  logger.info('********************************')
                ...
                syncNotes = 'contact create/update failed with error: ' + errorObj["sync_notes"];
            }
    ...
    _handleStgContact = async function (contactParam, customerCompanyIdMapping) {
        let { missedRequiredFields, stgContact, formattedData, contactParams, syncStatus, syncNotes } = contactParam;
        const formatteddata = formattedData;
        let contact_id = formatteddata.contact_id;
        if (missedRequiredFields.length > 0) {
            syncStatus = 'F';
            syncNotes = "Missing required fields: " + JSON.stringify(missedRequiredFields);
        }else if(!customerCompanyIdMapping.hasOwnProperty(contact_id)){
            syncNotes = 'no company found for this customer.';
            syncStatus = 'F';
        } else {
            try {
                let contactInfo = {};
                ...
                //  logger.info( '2222233333' +  JSON.stringify(contactInfo))
                syncNotes = 'contact create/update successfully';
                let ac_contact_id = contactInfo.id;
                let ac_company_id = customerCompanyIdMapping[contact_id];
                let assignResult = await _assignCustomerToCompany(ac_contact_id, ac_company_id)
                logger.info('66666' + JSON.stringify(assignResult))
                syncNotes = assignResult.sync_notes;
                syncStatus = assignResult.sync_status;
    ...
    _assignCustomerToCompany = async function (customer_id, company_id) {
        let syncNotes = 'Customer assign to company successfully';
        let syncStatus = 'O';
        try {
            const param = {
                "customer": {
            ...
            logger.info('55555' + JSON.stringify(param))
           let contactInfo = await oauth.put(constants.CONTACT_URI+'/'+customer_id, param);

        } catch (error) {
            logger.error(error)
            let errorObj = _handleError(error, syncStatus, syncNotes);
            syncStatus = errorObj["sync_status"]
            syncNotes = 'Customer assign to company failed with error: ' + errorObj["sync_notes"];
        }
        return {
            "sync_status": syncStatus,
            "sync_notes": syncNotes,
